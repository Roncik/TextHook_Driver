...
PVOID get_system_module_base(const char* module_name)
{
	...
	PRTL_PROCESS_MODULES modules = (PRTL_PROCESS_MODULES)ExAllocatePoolWithTag(NonPagedPool, bytes, 0x6E696767);
...
{
	if (!RtlCopyMemory(address, buffer, size)) // same as memcpy
	{
		return false;
	}
...
bool get_module_base_x64(PEPROCESS proc, UNICODE_STRING module_name, COMMS_MEMORY* towrite)
{
	...
	for (PLIST_ENTRY list = (PLIST_ENTRY)pLdr->InLoadOrderModuleList.Flink; list != &pLdr->InLoadOrderModuleList; list = (PLIST_ENTRY)list->Flink)
	{
		...
		if (RtlCompareUnicodeString(&pEntry->BaseDllName, &module_name, TRUE) == NULL)
		{
			...
			KeUnstackDetachProcess(&state);
			towrite->base_address = baseAddress;
			towrite->size = ModuleSize;
			return true;
		}
	}
